...
import com.fuzzy.service.FullRuleFuzzy.FullRuleCalculator;
import com.fuzzy.service.ModifiedFuzzy.ModifiedFuzzyCalculator;
import com.fuzzy.service.ProposedFuzzy.ProposedFuzzyCalculator;
import com.fuzzy.service.TfIdfFuzzy.TfIdfFuzzyCalculator;
import com.fuzzy.service.TfIdfNonFuzzy.TfIdfCalculator;
...
public class FuzzyService {
    ...
    public List<TfIdfWithArticleIndex> doSearch(@WebParam(name = "querry") String querry) throws JAXBException{
        ...
        List<String> categoryCollection = new ArrayList<>();
        ...
        try {
            ...
            rs = stmt.executeQuery("SELECT id, judul, isi, kategori FROM artikel ORDER BY id ASC");
        ...
        ArrayList<Double> idfArrayTitle = new ArrayList<>();
        ArrayList<Double> idfArrayContent = new ArrayList<>();
        int i = 0;
        FullRuleCalculator calculator = new FullRuleCalculator();
        while(i < query.size()){
            String temp = query.get(i);
            idfArrayTitle.add(calculator.idf(titles, temp));
            idfArrayContent.add(calculator.idf(documents, temp));
            i++;
        }
        i= 0;
        while (i < documents.size()) {
            ...
            double titleValue = 0;
            while (j < query.size()) {
                tfIdfValue += calculator.calculate(documents.get(i), documents, query.get(j), query, idfArrayContent.get(j));
                titleValue += calculator.calculate(titles.get(i), titles, query.get(j), query, idfArrayTitle.get(j));
                j++;//increment
            }
        ...
        ArrayList<TfIdfWithArticleIndex> arrayToSort = new ArrayList<TfIdfWithArticleIndex>();
                i = 0;
                while (i < tfIdf.size()) {
                    TfIdfWithArticleIndex tempOfTfIdf = new TfIdfWithArticleIndex(i, tfIdf.get(i), titleCollection.get(i), contentCollection.get(i), categoryCollection.get(i));
    ...
    public List<TfIdfWithArticleIndex> getCategory(@WebParam(name = "category") String category) {
        MyDBConnection mdbc = new MyDBConnection();
        mdbc.init();
        Connection conn = mdbc.getMyConnection();
        java.sql.Statement stmt = null;
        ResultSet rs = null;
        List<TfIdfWithArticleIndex> result = new ArrayList<>();
        try {
            stmt = conn.createStatement();
        
        rs = stmt.executeQuery("SELECT id, judul, isi, kategori FROM artikel WHERE kategori='" + category + "'");
        ...
        while(rs.next()){
            ...
            String documentContent = rs.getString("isi");
            String documentCategory = rs.getString("kategori");
            TfIdfWithArticleIndex tempObject = new TfIdfWithArticleIndex(documentId, "0", documentTitle, documentContent, documentCategory);
     ...
     * Web service operation
     */
    @WebMethod(operationName = "getRandom")
    public List getRandom(@WebParam(name = "input") String input) {
        MyDBConnection mdbc = new MyDBConnection();
        mdbc.init();
        Connection conn = mdbc.getMyConnection();
        java.sql.Statement stmt = null;
        ...
        rs = stmt.executeQuery("SELECT id, judul, isi, kategori FROM artikel ORDER BY RAND() LIMIT 20");
